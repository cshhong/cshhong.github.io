...
### Inferring the optimal trajectory
Remember from equation (1) that we are interested in the expected sum of rewards from a distribution of trajectories, given policy parameters. 
From our PGM formulation with the optimality variable, we can infer the posterior probability of observing a trajectory given optimality for all steps in the trajectory, which will serve as a useful intermediary step.

$$
\begin{aligned}
\textcolor{blue}{\underbrace{p(\tau \mid o_{1:T})}_{\textcolor{black}{\text{Probability of observing a given trajectory given that all steps are optimal}}}}
&\propto p(\tau, o_{1:T}) \\
&= p(s_1) \prod_{t=1}^T p(O_t = 1 \mid s_t, a_t) p(s_{t+1} \mid s_t, a_t) \\
&= p(s_1) \prod_{t=1}^T \exp(r(s_t, a_t)) p(s_{t+1} \mid s_t, a_t) \\
&= \textcolor{blue}{\underbrace{p(s_1) \prod_{t=1}^T p(s_{t+1} \mid s_t, a_t)}_{\textcolor{black}{\text{Probability of trajectory occurring according to the dynamics}}}} 
   \textcolor{blue}{\underbrace{\exp \left( \sum_{t=1}^T r(s_t, a_t) \right)}_{\textcolor{black}{\text{Exponential of the total reward along the trajectory}}}}.
\end{aligned}
$$
